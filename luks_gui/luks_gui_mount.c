...
int main () {
    ...
    if (gtk_dialog_run (GTK_DIALOG (dialog)) == GTK_RESPONSE_ACCEPT) {
        char cmd[3000] = "/usr/bin/sudo /usr/sbin/cryptsetup luksOpen `/usr/bin/sudo /usr/sbin/losetup --show -f \"";
        ...
        execl("/usr/bin/xterm", "/usr/bin/xterm", "-e", "/usr/bin/bash", "-c", cmd, (char *) 0);
    }
